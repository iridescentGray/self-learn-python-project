...
if __name__ == "__main__":
    ...
    # dpi-     设置绘图的分辨率
    # facecolor- 设置画布的背景色
    line_chart_in_the_same_coordinate_figure = plt.figure(
        figsize=(8, 4), dpi=120, facecolor="darkgray"
    )
    logging.info(f"figure return a {type(line_chart_in_the_same_coordinate_figure)}")

    # 三个参数分别用来指定整个画布分成几行几列以及当前坐标系的索引,这三个参数的默认值都是1
    line_chart_in_the_same_coordinate = (
        line_chart_in_the_same_coordinate_figure.add_subplot(2, 2, 1)
    )  # 使用add_subplot给画布创建一个或多个子图
    ...
    line_chart_in_the_same_coordinate_plot = line_chart_in_the_same_coordinate.plot(
        x, y2, linewidth=2, marker="*", color="blue"
    )
