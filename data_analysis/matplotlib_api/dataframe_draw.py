...
if __name__ == "__main__":
    ...
    dataframe_from_excel["销售额"] = (
        dataframe_from_excel["售价"] * dataframe_from_excel["销售数量"]
    )
    ...
    sales_revenue = pd.pivot_table(
        dataframe_from_excel, index="销售区域", values="销售额", aggfunc="sum"
    )
    # 柱状图
    ...
    pd.pivot_table(
        dataframe_from_excel, index="销售区域", values="销售额", aggfunc="sum"
    ).sort_values(by="销售额", ascending=False).plot(
        figsize=(6, 6),
        kind="pie",
        y="销售额",
        autopct="%.2f%%",
